...
async function main(argv = []) {
	...
	function renderProgress(entry, groups) {
		...
		let str = [
			...
			'ms]',
			'[' + (entry.totalSize.dirs / 1024 / 1024).toFixed(2),
			'Mb /',
			(entry.totalSize.files / 1024 / 1024).toFixed(2),
			'Mb]',
			'[' + `${parseInt(avgRam / 1024 / 1024).toLocaleString()}`,
			'Mb RAM]',
			'[!' + entry.errors.size + ']',
			'(' + entry.file.path + ')',
		].join(' ')
	...
	const groups = {
		...
		images: (entry) =>
			['.png', '.jpg', '.jpeg', '.gif', '.svg', '.webp'].includes(
				extname(entry.file.name).toLocaleLowerCase(),
			),
